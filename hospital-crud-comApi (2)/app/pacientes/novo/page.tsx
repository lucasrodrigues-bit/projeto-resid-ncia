...
export default function NovoPacientePage() {
  ...
  if (isLoading) return
  setIsLoading(true)
  showToast({ title: "Enviando dados...", description: "Os dados do paciente estão sendo enviados para a API." })
    ...
    if (celular && !/^\+55 \(\d{2}\) \d{4,5}-\d{4}$/.test(celular))
    ...
    if (errors.length) {
  showToast({ title: "Corrija os campos", description: errors[0] })
      setIsLoading(false)
      return
    }
    ...
    try {
      ...
      showToast({
        ...
        description: "Paciente cadastrado com sucesso. Você será redirecionado.",
      })
      setTimeout(() => {
